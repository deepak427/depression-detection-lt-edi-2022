...
def get_data(data_split, use_shuffle=False, without_label=False):
    ...
    df = pd.read_csv(f'D:\dipu\Hackthons\depression-detection-lt-edi-2022\data\original_dataset\{data_split}.tsv', sep='\t', header=0)
...
def get_preprocessed_data(data_split, use_shuffle=False):
    df = pd.read_csv(f'D:\dipu\Hackthons\depression-detection-lt-edi-2022\data\original_dataset\{data_split}.csv', header=0, lineterminator='\n')
    if use_shuffle:
        return shuffle(df)
    return df
